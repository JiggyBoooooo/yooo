...
export default async (req) => {
  ...
  try {
    ...
    const destination = String(body?.delivery?.destination || '').trim();
    ...
    const customerEmail = typeof body?.customer?.email === 'string' ? body.customer.email.trim().slice(0, 320) : undefined;
    const customerName = typeof body?.customer?.name === 'string' ? body.customer.name.trim().slice(0, 200) : '';
